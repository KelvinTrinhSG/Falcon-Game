...
function ShopController:Restock(player: Player, suppressNotification: boolean?, isFirstTime: boolean?)
	...
	-- Cách A: turret Titan có cờ InShop (LimitedItems) roll chung vào kho shop này,
	-- GIỐNG HỆT 9 turret thường — không kho global, không kẹp gì.
	-- Turret không có InShop (nhóm Upgraded) -> KHÔNG set newStock[itemId] (để nil) -> client ẩn card.
...
local function onPurchaseRequest(player: Player, itemId: string)
	local profile = PlayerController:GetProfile(player)
	-- Cách A: turret Titan có cờ InShop nằm ở bảng LimitedItems, xử lý y hệt turret thường.
	local config = ItemConfigurations[itemId] or LimitedItems[itemId]
