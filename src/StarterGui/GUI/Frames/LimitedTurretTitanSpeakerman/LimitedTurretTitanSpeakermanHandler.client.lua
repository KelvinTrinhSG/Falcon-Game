...
local hudButton = playerGui:WaitForChild("GUI"):WaitForChild("HUD"):WaitForChild("Right"):WaitForChild("LimitedTurretTitanSpeakerman")
...
local limitedItemId = "TitanSpeakerman"
local limitedItemConfig = LimitedItems[limitedItemId]
local MAX_STOCK = 999

local function updateDisplay(stock: number)
	if not limitedItemConfig then
		warn("[LimitedTurretTitanSpeakermanHandler] Could not find config for '" .. limitedItemId .. "' in LimitedItems.")
		return
	end
