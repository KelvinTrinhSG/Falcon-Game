--#ReplicatedStorage>Modules>ItemConfigurations
local ItemConfigurations = {
	--BLOCKS
	RockBlock = {
		DisplayName = "Rock Block",
		Type = "Blocks",
		Price = 100,
		ImageId = "rbxassetid://136783950073385",
		ProductID = 3710658637,
		Unlimited = false,
		Chance = 100,
		StockAmount = {Min = 1, Max = 7},
		Health = 50,
	},

	ConcreteBlock = {
		DisplayName = "Concrete Block",
		Type = "Blocks",
		Price = 200,
		ImageId = "rbxassetid://114128595436055",
		ProductID = 3710658669,
		Unlimited = false,
		Chance = 95,
		StockAmount = {Min = 1, Max = 7},
		Health = 100,
	},

	IceBlock = {
		DisplayName = "Ice Block",
		Type = "Blocks",
		Price = 300,
		ImageId = "rbxassetid://103260168489888",
		ProductID = 3710658705,
		Unlimited = false,
		Chance = 90,
		StockAmount = {Min = 1, Max = 7},
		Health = 150,
	},

	FireBlock = {
		DisplayName = "Fire Block",
		Type = "Blocks",
		Price = 400,
		ImageId = "rbxassetid://77293299946865",
		ProductID = 3710658731,
		Unlimited = false,
		Chance = 85,
		StockAmount = {Min = 1, Max = 7},
		Health = 200,
	},

	LavaBlock = {
		DisplayName = "Lava Block",
		Type = "Blocks",
		Price = 5000,
		ImageId = "rbxassetid://115988803054884",
		ProductID = 3710658765,
		Unlimited = false,
		Chance = 80,
		StockAmount = {Min = 1, Max = 7},
		Health = 1200,
	},

	ToxicBlock = {
		DisplayName = "Toxic Block",
		Type = "Blocks",
		Price = 7500,
		ImageId = "rbxassetid://77307474711071",
		ProductID = 3710658796,
		Unlimited = false,
		Chance = 75,
		StockAmount = {Min = 1, Max = 7},
		Health = 7200,
	},

	GoldBlock = {
		DisplayName = "Gold Block",
		Type = "Blocks",
		Price = 10000,
		ImageId = "rbxassetid://76880802576509",
		ProductID = 3710658838,
		Unlimited = false,
		Chance = 70,
		StockAmount = {Min = 1, Max = 7},
		Health = 14400,
	},

	PlasmaBlock = {
		DisplayName = "Plasma Block",
		Type = "Blocks",
		Price = 12500,
		ImageId = "rbxassetid://116185531123879",
		ProductID = 3710658872,
		Unlimited = false,
		Chance = 65,
		StockAmount = {Min = 1, Max = 7},
		Health = 28800,
	},

	CyberBlock = {
		DisplayName = "Cyber Block",
		Type = "Blocks",
		Price = 25000,
		ImageId = "rbxassetid://94711260291584",
		ProductID = 3710658910,
		Unlimited = false,
		Chance = 60,
		StockAmount = {Min = 1, Max = 7},
		Health = 57600,
	},

	TitanBlock = {
		DisplayName = "Titan Block",
		Type = "Blocks",
		Price = 50000,
		ImageId = "rbxassetid://129725207947239",
		ProductID = 3710658939,
		Unlimited = false,
		Chance = 55,
		StockAmount = {Min = 1, Max = 7},
		Health = 115200,
	},

	--TURRETS
	-- Order: CameraGuy > EngineerCameraGuy > SpeakerGuy > TvGuy
	--        > LargeScientistCameraman > LargeSpeakerGuy > LargeTvGuy > LaserCameramanCar > TitanCameraGuy
	CameraGuy = {
		DisplayName = "Camera Guy",
		Type = "Turrets",
		Price = 200,
		ImageId = "rbxassetid://87434168468506",
		ProductID = 3710656644,
		Unlimited = false,
		Chance = 100,
		StockAmount = {Min = 1, Max = 5},
		Damage = 50,
		Range = 10,
		Cooldown = 1,
		Health = 5000000000,
	},

	EngineerCameraGuy = {
		DisplayName = "Engineer Camera Guy",
		Type = "Turrets",
		Price = 600,
		ImageId = "rbxassetid://127153357548293",
		ProductID = 3710656704,
		Unlimited = false,
		Chance = 90,
		StockAmount = {Min = 1, Max = 5},
		Damage = 50,
		Range = 30,
		Cooldown = 2,
		Health = 5000000000,
		TargetingMode = "ClosestToEnd",
	},

	SpeakerGuy = {
		DisplayName = "Speaker Guy",
		Type = "Turrets",
		Price = 800,
		ImageId = "rbxassetid://95942695589812",
		ProductID = 3710656746,
		Unlimited = false,
		Chance = 80,
		StockAmount = {Min = 1, Max = 5},
		Damage = 45,
		Range = 10,
		Cooldown = 0.5,
		Health = 5000000000,
		TargetingMode = "HighestHP",
	},

	TvGuy = {
		DisplayName = "TV Guy",
		Type = "Turrets",
		Price = 400,
		ImageId = "rbxassetid://86656983607357",
		ProductID = 3710656794,
		Unlimited = false,
		Chance = 95,
		StockAmount = {Min = 1, Max = 5},
		Damage = 50,
		Range = 10,
		Cooldown = 2,
		Health = 5000000000,
		TargetingMode = "HighestSpeed",
		SlowEffect = { Duration = 2, SpeedMultiplier = 0.5 },
	},

	LargeScientistCameraman = {
		DisplayName = "Large Scientist Cameraman",
		Type = "Turrets",
		Price = 20000,
		ImageId = "rbxassetid://111983598950365",
		ProductID = 3710656883,
		Unlimited = false,
		Chance = 70,
		StockAmount = {Min = 1, Max = 5},
		Damage = 700,
		Range = 35,
		Cooldown = 1.8,
		Health = 5000000000,
		TargetingMode = "ClosestToEnd",
	},

	LargeSpeakerGuy = {
		DisplayName = "Large Speaker Guy",
		Type = "Turrets",
		Price = 25000,
		ImageId = "rbxassetid://127572002232247",
		ProductID = 3710656915,
		Unlimited = false,
		Chance = 65,
		StockAmount = {Min = 1, Max = 5},
		Damage = 630,
		Range = 15,
		Cooldown = 0.45,
		Health = 5000000000,
		TargetingMode = "HighestHP",
	},

	LargeTvGuy = {
		DisplayName = "Large TV Guy",
		Type = "Turrets",
		Price = 15000,
		ImageId = "rbxassetid://134355068425451",
		ProductID = 3710656961,
		Unlimited = false,
		Chance = 75,
		StockAmount = {Min = 1, Max = 5},
		Damage = 700,
		Range = 15,
		Cooldown = 1.8,
		Health = 5000000000,
		TargetingMode = "HighestSpeed",
		SlowEffect = { Duration = 3, SpeedMultiplier = 0.5 },
	},

	LaserCameramanCar = {
		DisplayName = "Laser Cameraman Car",
		Type = "Turrets",
		Price = 10000,
		ImageId = "rbxassetid://140634982638027",
		ProductID = 3710657003,
		Unlimited = false,
		Chance = 80,
		StockAmount = {Min = 1, Max = 5},
		Damage = 700,
		Range = 15,
		Cooldown = 0.9,
		Health = 5000000000,
	},

	TitanCameraGuy = {
		DisplayName = "Titan Camera Guy",
		Type = "Turrets",
		Price = 80000,
		ImageId = "rbxassetid://121254234742118",
		ProductID = 3710657047,
		Unlimited = false,
		Chance = 60,
		StockAmount = {Min = 1, Max = 3},
		Damage = 8000,
		Range = 14,
		Cooldown = 0.9,
		Health = 5000000000,
		ExplosionRadius = 6,
	},

	TitanTVMan = {
		DisplayName = "Titan TV Man",
		Type = "Turrets",
		Price = 120000,
		ImageId = "rbxassetid://79524847091825",
		ProductID = 3710673684,
		Unlimited = false,
		Chance = 55,
		StockAmount = {Min = 1, Max = 3},
		Damage = 8000,
		Range = 18,
		Cooldown = 1.8,
		Health = 5000000000,
		TargetingMode = "HighestSpeed",
		SlowEffect = { Duration = 6, SpeedMultiplier = 0.5 },
	},

	TitanSpeakerman = {
		DisplayName = "Titan Speakerman",
		Type = "Turrets",
		Price = 200000,
		ImageId = "rbxassetid://77372914868800",
		ProductID = 3710673766,
		Unlimited = false,
		Chance = 45,
		StockAmount = {Min = 1, Max = 3},
		Damage = 7200,
		Range = 18,
		Cooldown = 0.45,
		Health = 5000000000,
		TargetingMode = "HighestHP",
	},

	UpgradedTitanCameraGuy = {
		DisplayName = "Upgraded Titan Camera Guy",
		Type = "Turrets",
		Price = 160000,
		ImageId = "rbxassetid://110200837070805",
		ProductID = 3710673827,
		Unlimited = false,
		Chance = 50,
		StockAmount = {Min = 1, Max = 3},
		Damage = 8000,
		Range = 45,
		Cooldown = 1.8,
		Health = 5000000000,
		TargetingMode = "ClosestToEnd",
	},

	SecretAgent = {
		DisplayName = "Secret Agent",
		Type = "Turrets",
		Price = 0,
		ImageId = "rbxassetid://0",
		ProductID = 0,
		Health = 5000000000,
		CashPerSecond = 2,
	},
}

local LimitedItems = {
}

local BaseConfigurations = {
	Core1 = { DisplayName = "Camera Post",     Price = 0,     Health = 100,  ImageId = "rbxassetid://138618181818045", ProductID = 0           },
	Core2 = { DisplayName = "Speaker Station", Price = 2000,  Health = 200,  ImageId = "rbxassetid://119650006445523", ProductID = 3710733502  },
	Core3 = { DisplayName = "Broadcast Tower", Price = 10000, Health = 400,  ImageId = "rbxassetid://72443890466175",  ProductID = 3710733557  },
	Core4 = { DisplayName = "Signal Fortress", Price = 25000, Health = 600,  ImageId = "rbxassetid://129302868825261", ProductID = 3710733607  },
	Core5 = { DisplayName = "Titan Dominion",  Price = 60000, Health = 800,  ImageId = "rbxassetid://117127516225593", ProductID = 3710733660  },
}

return {
	LimitedItems = LimitedItems,
	ItemConfigurations = ItemConfigurations,
	BaseConfigurations = BaseConfigurations
}